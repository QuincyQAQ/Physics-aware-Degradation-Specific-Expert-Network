...
parser.add_argument('-p', '--config', type=str, default='./options/inference/DeMoE.yml', help = 'Config file of prediction')
parser.add_argument('-c', '--checkpoints_path', type=str, default='./models/DeMoE.pt', 
                help="Checkpoints path")
parser.add_argument('-i', '--inp_path', type=str, default='./images/inputs', 
                help="Folder path")
parser.add_argument('-t', '--task', type=str, default='auto', choices=['auto', 'defocus', 'global_motion', 'synth_global_motion', 'local_motion', 'low_light'],
                    help="Task type")
...
def predict_folder():
    ...
    for path_img in path_images:
        ...
        with torch.no_grad():
            output_dict = model.module.forward(tensor, task = args.task)
            output = output_dict['output']
            # output = output[-1]
            if 'pred_labels' in output_dict:
